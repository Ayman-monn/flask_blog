...
class AdminController:
    ...
    def users_control(): 
        if current_user.username == cfg.OWNER_USERNAME: 
            pagination, user_list = Paginate(cfg.USERS_PER_PAGE, User, User.id.desc())
            return render_template("auth/users_control.jinja", title="المستخدمين", pagination=pagination, users_list=user_list) 
    ...
    def role_grant(user_id): 
        ...
        if current_user.username == cfg.OWNER_USERNAME: 
            ...
            user.is_admin = True
            db.session.commit()
            flash(f"تم منح صلاحيات الإدارة للمستخدم {user.username}", "warning")
            return redirect(url_for("auth_controller.users_control"))
        else: 
            flash("العملية غير ممكنة", "warning") 
    ...
    def role_revoke(user_id):
        ...
        if current_user.username == cfg.OWNER_USERNAME: 
            if user.is_admin: 
                user.is_admin=False
                db.session.commit() 
                flash(f"تم سحب صلاحية الإدارة من المستخدم {user.username}", 'warinig')
            else: 
                flash(f"{user.username} لايملك صلاحية اللإدارة", 'warinig')
            return redirect(url_for('auth_controller.users_control'))
