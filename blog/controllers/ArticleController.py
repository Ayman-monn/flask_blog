...
from blog.utils.ArticleUtlis import save_image
...
class ArticleController: 
    ...
    def article_add(): 
        if current_user.is_admin is False: 
            flash("لا تملك صلاحية الوصول للصفحة المطلوبة", "danger")
        ...
        if form.validate_on_submit(): 
            if form.article_img.data: 
                ...
                image_name = None 
            new_article = Article(user_id = current_user.id, title =form.title.data, content=form.content.data, article_img=image_name)
    ...
    def article_update(id): 
        ...
        return render_template("articles/article_add.jinja", form=form, legend="تعديل المقالة:", title="تعديل المقالة")
    ...
    def article_delete(id): 
        if current_user.is_admin:
            ...
            flash("لا تمتلك صلاحية الوصول للصفحة المطلوبة", "warning")
            return redirect(url_for('main_controller.home')) 
    ...
    def articles_list(): 
        if current_user.is_admin:
            ...
            return render_template("articles/articles_list.jinja",
                                    articles_list=articles_list, pagination=pagination, title="لائحة المقالات") 
        else: 
            flash("لا تمتلك صلاحية الوصول للصفحة المطلوبة", "warning")
            return redirect(url_for('main_controller.home')) 
